...
pub fn build(b: *std.Build) void {
    ...
    // Copy Python files to the output directory
    const python_files_step = b.addInstallDirectory(.{
        .source_dir = b.path("src/python"),
        .install_dir = .bin,
        .install_subdir = "",
    });

    // Make executable depend on Python files being copied
    b.getInstallStep().dependOn(&python_files_step.step);
    ...
    // Add environment variable to help the executable find Python files
    run_cmd.setEnvironmentVariable("PYTHON_MODULE_PATH", "python");
